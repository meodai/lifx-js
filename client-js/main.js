...
var $cam = $('#cam');
var $img = $('#camOutputImg');
var imageData, data, quality = 20;
var hex = null;

var loop = function(){
  if( localMediaStream ){
    ctx.drawImage($cam[0], 0, 0);
    //$img.attr('src', $canvas[0].toDataURL('image/jpg'));
    imageData = ctx.getImageData(0, 0, 640, 480).data;

    var pixelArray = [];
    for (var i = 0, offset, r, g, b, a; i < (640 * 480); i = i + quality) {
        offset = i * 4;
...
userMedia.getUserMedia(camOptions, function(stream){
  $cam[0].src = window.URL.createObjectURL(stream);
  $cam[0].onloadedmetadata = function(e) {
    $cam[0].play();
